...
/**
 * Fixed dock in the bottom trailing corner: a WhatsApp shortcut, a
 * back-to-top control that appears once the visitor has scrolled, and the
 * accessibility widget. Sits above the compare tray when that is showing.
 */
export default function FloatingDock({ waNumber }: { waNumber: string }) {
  ...
  return (
    <div className="fixed bottom-5 end-4 z-[60] flex flex-col items-center gap-3 print:hidden">
      {showTop && (
        <button
          type="button"
          onClick={toTop}
      ...
      <a
        ...
        className="flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg ring-1 ring-black/5 hover:brightness-95"
      >
